...
var _utils3 = require("../../core/server/utils");
  ...
  if (opts.dev) {
    ...
    if (opts.ssl) {
      ...
      const {
        ...
        OSD_KEY_PATH,
        OSD_CERT_PATH
      } = require('@osd/dev-utils');

      const customOpenSearchHosts = opts.opensearch ? opts.opensearch.split(',') : [].concat(get('opensearch.hosts') || []);
      ...
      const opensearchHosts = (customOpenSearchHosts.length > 0 && customOpenSearchHosts || ['https://localhost:9200']).map(hostUrl => {
